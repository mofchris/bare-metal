...
Draw performance (GFLOPS, log scale) against arithmetic intensity (FLOPs per
byte, log scale). Two lines bound everything the machine can ever do:

- a **slanted line** through the origin with slope = peak bandwidth: at
  intensity AI, streaming memory can sustain at most `AI × bandwidth` FLOPs/s;
- a **flat line** at peak compute: no kernel exceeds the arithmetic units.

The machine's limit at any intensity is the _lower_ of the two:
...
GFLOPS, 100 GB/s) the ridge sits at 4 FLOPs/byte.
...
1. **Under the slanted roof** (memory-bound, e.g. elementwise add at 0.08
   ...
   moves that help are _moving fewer bytes_ — better reuse, fusion, smaller
   dtypes (fp16 halves the bytes and doubles effective intensity).
...
3. **Well below either roof**: the kernel isn't hitting _any_ hardware limit
   ...
   defeating the prefetcher, thread imbalance). Fix the code before blaming
   the machine.
...
The basic roofline assumes perfect overlap of compute and memory traffic and
uses _peak_ numbers that real code rarely sustains. Fancier versions add
ceilings (no-SIMD ceiling, single-core ceiling, cache-level bandwidths). Lab
L1.3 builds the basic one for your machine from _measured_ peaks — a roofline
built from marketing numbers is a ceiling you'll never touch.
...
intensity), why LLM decode is bandwidth-bound (M7), and GPU kernel tuning
(M8). If you can place a workload on a roofline and name the binding budget,
you already think like an MLSys engineer.
