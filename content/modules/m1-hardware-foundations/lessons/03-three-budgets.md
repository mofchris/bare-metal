...
(AI), in FLOPs per byte. It tells you which budget the code lives on.

Work the examples by hand once — this is the skill:

- **Elementwise add** `c[i] = a[i] + b[i]` (fp32): per element, 1 FLOP;
  bytes: read 4+4, write 4 = 12. AI ≈ **0.08 FLOP/byte**.
...
FLOPs/byte**. Code below that intensity cannot use all the compute — the
memory system can't feed it. Code above it can. So on this laptop:

- Elementwise add at 0.08 → memory-bound, ~50× below break-even. Its speed
  limit is 100 GB/s ÷ 12 B × 1 FLOP ≈ 8 GFLOPS — 2% of peak, and **no
  amount of extra compute, cores, or clock speed changes that**.
- Large matmul at N/6 → compute-bound once N is a few dozen. Worth
  optimizing arithmetic.

## Why ML engineers care daily

Most of a neural network's operations by _count_ are low-intensity
(activations, normalization, elementwise everything) while most of its FLOPs
sit in high-intensity matmuls. That's why kernel fusion (M10) exists — gluing
elementwise ops onto the matmul before/after them so their bytes ride along
free — and why "the GPU is at 100% utilization" can still mean "the expensive
units are idle, waiting on memory." Utilization of _what_ is the question
